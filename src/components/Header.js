...
export default function Header({
    ...
    setN2,
    boardReloadHandler,
    obstacleReloadHandler,
}) {
    const [selectedOptions, setSelectedOptions] = useState(
        'Dijkstras Algorithom'
    );
    ...
    const updateGrid = () =>
        setGridSize({
            width: inputStore.gridWidth,
            height: inputStore.gridHeight,
        });
    ...
    return (
        <div className="flex outline outline-1 h-fit ">
            <div className="border m-1 bg-gray-200 m-2 p-2 rounded">
                <div className="border-b bg-white pl-3">
                    Algorithom Controls
                </div>
                <select
                    className="outline outline-1 outline-gray-500 rouned p-1 h-fit w-full mt-2"
                    onChange={optionHandler}
                >
                    <option>Dijkstras Algorithom</option>
                </select>
                <button
                    className="START BUTTON outline outline-1 outline-gray-500 rounded p-1 m-2 h-fit bg-white"
                    onClick={startAlgoButtonHandler}
                >
                    Start Simulation
                </button>
                <button
                    className="CLEAR BUTTON outline outline-1 outline-gray-500 rounded p-1 m-2 h-fit bg-white"
                    onClick={boardReloadHandler}
                >
                    Clear
                </button>
            </div>
            <div className="bg-gray-200 rounded m-2 p-2">
                ...
                <div className="border bg-gray-200 rounded flex">
                    ...
                    <button
                        className="outline outline-1 rounded-sm bg-white m-1 px-2"
                        onClick={updateGrid}
                    >
                        Apply
                    </button>
                ...
                <div className="bg-gray-100 border">
                    <label className="bg-gray-100 ml-0.5">
                        Min-Value is 4, Max-Value is 99
                    </label>
                </div>
            </div>
            <div className="bg-gray-200 rounded m-2 p-2">
            ...
            <div className="bg-gray-200 rounded m-2 p-2">
                ...
                <div className="border bg-gray-200 rounded w-fit w-36">
                    <select
                        className="outline outline-1 outline-gray-500 m-1 p-1 h-8 w-32 mt-2 grow "
                        onChange={optionHandler}
                    >
                        <option>Wall</option>
                    </select>
                    <div className="grow flex justify-center">
                        <button
                            className="outline outline-1 outline-gray-500 rounded self-ce p-1 m-1 h-fit bg-white"
                            onClick={obstacleReloadHandler}
                        >
                            Clear Obstacles
                        </button>
                    </div>
